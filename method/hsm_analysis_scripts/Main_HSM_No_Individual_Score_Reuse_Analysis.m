%% CS-CGA HSM analysis without individual-level score reuse.
root_dir = fileparts(mfilename('fullpath'));
addpath(fullfile(root_dir, 'tools', 'HSMAnalysis'), '-begin');
setup_HSM_analysis_paths(root_dir);

dataset_override = strtrim(getenv('HSM_NO_INDIVIDUAL_SCORE_REUSE_DATASETS'));
...
result_dir = fullfile(root_dir, 'results', ['HSM_no_individual_score_reuse_analysis_', run_id]);
if ~exist(result_dir, 'dir')
    mkdir(result_dir);
end

summary_file = fullfile(result_dir, 'hsm_no_individual_score_reuse_summary.csv');
write_no_individual_score_reuse_header(summary_file);

fprintf('HSM no-individual-score-reuse analysis output: %s\n', summary_file);
...
for dataset_id = 1:numel(datasets)
    ...
    fprintf('[%s] Dataset=%s-%d Variant=NoIndividualScoreReuse-HSM-paper\n', ...
        datestr(now), BN_Name, sample_size);
    t_start = tic;

    [dag, best_score, ~, iterations, cache_stats] = HSM_no_individual_score_reuse_analysis_process( ...
    ...
    row.variant = 'NoIndividualScoreReuse-HSM-paper';
    row.cacheMode = 'hsm-no-individual-score-reuse';
    ...
    print_no_individual_score_reuse_row(row);
    append_no_individual_score_reuse_row(summary_file, row);
end

fprintf('HSM no-individual-score-reuse analysis finished: %s\n', summary_file);

function write_no_individual_score_reuse_header(filename)
...
function append_no_individual_score_reuse_row(filename, row)
...
function print_no_individual_score_reuse_row(row)
