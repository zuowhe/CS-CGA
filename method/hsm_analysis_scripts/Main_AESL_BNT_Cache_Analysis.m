%% AESL-GA analysis with the standard BNT local score cache.
root_dir = fileparts(mfilename('fullpath'));
addpath(fullfile(root_dir, 'tools', 'AESLHSMAnalysis'), '-begin');
setup_AESL_HSM_analysis_paths(root_dir);

dataset_override = strtrim(getenv('AESL_BNT_ANALYSIS_DATASETS'));
...
sample_size_override = strtrim(getenv('AESL_BNT_ANALYSIS_SAMPLE_SIZE'));
...
variant = struct();
variant.name = 'BNT-Cache';
variant.mode = 'bnt';
variant.parentCacheMultiplier = [];
variant.parentCacheSize = [];
variant.largeHashBytes = [];

run_id = datestr(now, 'yyyymmdd_HHMMSS');
result_dir = fullfile(root_dir, 'results', ['AESL_BNT_cache_analysis_', run_id]);
if ~exist(result_dir, 'dir')
    mkdir(result_dir);
end

summary_file = fullfile(result_dir, 'aesl_bnt_cache_summary.csv');
write_aesl_bnt_cache_header(summary_file);

fprintf('AESL BNT-cache analysis output: %s\n', summary_file);
...
for dataset_id = 1:numel(datasets)
    ...
    [dag, best_score, ~, iterations, cache_stats] = AESL_HSM_analysis_process( ...
        super_structure, data, N, M, MP, alpha, d, scoring_fn, bnet, variant);
    ...
    print_aesl_bnt_cache_row(row);
    append_aesl_bnt_cache_row(summary_file, row);
end

fprintf('AESL BNT-cache analysis finished: %s\n', summary_file);

function write_aesl_bnt_cache_header(filename)
...
function append_aesl_bnt_cache_row(filename, row)
...
function print_aesl_bnt_cache_row(row)
fprintf(['RESULT dataset=%s variant=%s runtime=%.3fs globalHit=%.5f ' ...
    'localHit=%.5f overwrites=%d globalKB=%.3f localKB=%.3f totalKB=%.3f\n'], ...
    row.dataset, row.variant, row.runtimeSec, row.globalHitRate, ...
    row.localHitRate, row.overwrites, row.globalCacheKB, row.localCacheKB, row.totalCacheKB);
end
