function run_CS_CGA_ablation_on_dataset(Variant, Ds, BN_Name, N, M, MP, tour, bnet, trial, scoring_fn)
cfg = CS_CGA_ablation_config(Variant);
AlgoName = cfg.output_name;
fprintf('Running ablation... %s_%s%s - N=%s M=%s [%s]\n', ...
    AlgoName, BN_Name, num2str(Ds), num2str(N), num2str(M), datestr(now));
fprintf('Iter   F1 Score   Sensitivity   Specificity   Precision   SHD    BIC Score      Exe Time  #Generations TP  TP2  FN  FP  TN\n');
eoer = cell(trial, 1);
...
for T = 1:trial
    ...
    eoer{T} = -1 * ones(1, 9);
    tStart = tic;
    p_value = PCR_compute_pvalue_matrix(TrainData{T}, bnet, 0.01);
    [dag, eoer{T}(1, 4), conv(T), eoer{T}(1, 6)] = ...
        CS_CGA_ablation_search(TrainData{T}, N, M, MP, scoring_fn, bnet, tour, saves_result_file, p_value, Variant);
    eoer{T}(1, 5) = toc(tStart);
    [eoer{T}(1, 1), eoer{T}(1, 2), eoer{T}(1, 3), eoer{T}(1, 8), eoer{T}(1, 7), TP, TP2, FN, FP, TN] = ...
        eval_dags_adjust(dag, bnet.dag, 1);
...
fprintf(saved_file, 'Std,%9.5f,%9.5f,%9.5f,%9.5f,%5.3f,%11.3f,%11.3f,%7.3f\n', ...
    eoer_std(1), eoer_std(2), eoer_std(3), eoer_std(8), eoer_std(7), eoer_std(4), eoer_std(5), eoer_std(6));
end
